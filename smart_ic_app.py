...
def ask_claude_json(sales_context, plan_rules_text, question, column_info=""):
    ...
    === DATA STRUCTURE ===
    The user has uploaded a file with these columns/tables: {column_info}
    Use ALL sheets provided in the data.
    ...
    {{
        "final_answer_summary": "A 1-sentence executive summary.",
        "key_stats": [
            {{"label": "Total Payout", "value": "$10,000", "delta": "+5% vs Target"}},
            {{"label": "Attainment", "value": "105%", "delta": "Above Quota"}}
        ],
        "detailed_logic_markdown": "Use Markdown Tables for calculations. Example: | Month | Sales | MBO | Total |\\n|---|---|---|---|\\n| Jan | $10 | $5 | $15 |",
        "rule_citations": ["Rule 6.4: Cyclone Relief - Adjusted March Baseline", "Rule 2.1: Accelerator applied >100%"],
        "chart_config": {{
            "type": "bar", 
            "title": "Monthly Payout Trend",
            "data": {{"Jan": 100, "Feb": 120, "Mar": 80}}
        }}
    }}
    Note: For 'chart_config', use 'line' type for time-series trends, 'bar' for categories.
    """
    ...
    try:
        ...
        if parsed_data:
            ...
            return {
                "final_answer_summary": "⚠️ AI Parsing Error.", 
        ...
        return {"final_answer_summary": "System Error", "detailed_logic_markdown": str(e), "chart_config": {}}

# ==========================================
# 2. DATA HANDLERS (MULTI-TAB SUPPORT)
# ==========================================
...
def process_sales_file(uploaded_file):
    ...
    try:
        if uploaded_file.name.endswith('.csv'):
            ...
            df = df.fillna(0)
            combined_context = df.head(400).to_csv(index=False)
            ...
            for sheet_name, sheet_df in xls.items():
                ...
                combined_context += f"\n\n=== SHEET: {sheet_name} ===\n"
                combined_context += sheet_df.head(200).to_csv(index=False)
                all_columns.extend([f"{sheet_name}.{c}" for c in sheet_df.columns])
                if len(sheet_df) > max_rows:
                    max_rows = len(sheet_df)
                    main_df = sheet_df

        columns_str = ", ".join([str(c) for c in all_columns])
        return main_df, combined_context, columns_str
...
if mode == "Demo Sandbox":
    ...
    current_sales_df = pd.read_csv(io.StringIO("""Date,Customer Name,Product Category,Sales Amount
2024-05-15,KENTUCKY CLINIC,Consumables,2800"""))
    current_plan_rules = get_default_plan()
    current_columns = "Date, Customer Name, Product Category, Sales Amount"
    ...
    if sales_file:
        ...
        if current_sales_df is not None:
             st.success(f"✅ Data Loaded (Includes multiple sheets if present)")
...
with tab1:
    if current_sales_df is not None and len(current_sales_df) > 0:
        ...
        for item in st.session_state.history:
            ...
            # 1. Executive Summary
            summary = res.get('final_answer_summary', 'Processing...')
            if "Error" in summary:
                st.error(summary)
            else:
            ...
            # 2. Key Metrics Row
            stats = res.get('key_stats', [])
            if stats:
                cols = st.columns(len(stats))
                for idx, stat in enumerate(stats):
                    cols[idx].metric(label=stat.get('label', 'Metric'), 
                                     value=stat.get('value', '0'), 
                                     delta=stat.get('delta', None))

            with st.expander("📊 See Logic, Rules & Charts", expanded=True):
                # 3. Rule Citations
                rules = res.get('rule_citations', [])
                if rules:
                    st.markdown("#### 📜 Plan Rules Applied")
                    for rule in rules:
                        st.info(f"ℹ️ {rule}")

                # 4. Detailed Logic (Markdown Table)
                logic = res.get('detailed_logic_markdown', 'No details provided.')
                st.markdown("#### 🧮 Calculation Ledger")
                st.markdown(logic)

                # 5. Dynamic Charts
                chart_conf = res.get('chart_config', {})
                if chart_conf and chart_conf.get('data'):
                    st.markdown(f"#### 📈 {chart_conf.get('title', 'Visual Evidence')}")
                    chart_data = chart_conf['data']
                    df_chart = pd.DataFrame(list(chart_data.items()), columns=["Category", "Value"]).set_index("Category")
                    
                    if chart_conf.get('type') == 'line':
...
with tab2:
    ...
    with st.expander("View Combined Sales Payload (All Sheets)"):
        st.text(current_sales_data_str[:5000] if current_sales_data_str else "Empty")