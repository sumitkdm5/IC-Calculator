...
def ask_claude_json(sales_context, plan_rules_text, question, column_info=""):
    ...
    Note: The data may contain multiple sheets (e.g., Sales vs MBO). Use all of them.
    ...
    If data is missing, check ALL sheets in the 'SALES & MBO DATA' section.
    """
...
def process_sales_file(uploaded_file):
    """
    Reads EVERY sheet in the Excel file and combines them into one context string.
    Returns: 
    1. Main DataFrame (largest sheet) for Insights.
    2. Combined String (all sheets) for AI.
    3. List of all columns found.
    """
    uploaded_file.seek(0)
    ...
    try:
        if uploaded_file.name.endswith('.csv'):
            ...
            for sheet_name, sheet_df in xls.items():
                ...
                # Append to Context String for AI
                combined_context += f"\n\n=== SHEET: {sheet_name} ===\n"
                ...
                # Keep largest sheet for 'Quick Insights' buttons
...
def generate_quick_insights(df_input):
    """
    Generates prompt suggestions based on the MAIN data sheet (Sales).
    """
    insights = []
...
if mode == "Demo Sandbox":
    ...
    # Dummy DF creation for insights (mocking the structure)
