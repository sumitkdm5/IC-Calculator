...
def clean_json_string(raw_text):
    """
    Robust function to find JSON and sanitize it.
    """
    # 1. extract content between outermost brackets
    start = raw_text.find('{')
    ...
    # 2. Remove "Control Characters" that break JSON (newlines, tabs inside strings)
    # We replace strict newlines with literal \n characters
    json_str = re.sub(r'(?<!\\)\n', r'\\n', json_str)
...
def ask_claude_json(sales_context, plan_rules_text, question, is_strict_mode=False):
    ...
    IMPORTANT:
    1. Do NOT use real newlines inside the JSON values. Use the escape sequence "\\n" for line breaks.
    ...
    try:
        ...
        # NUCLEAR OPTION: Attempt to parse
        ...
        if cleaned_json:
            try:
                ...
                # Fallback 1: Try strict mode (Python's strict=False allows some control chars)
                try:
                    return json.loads(cleaned_json, strict=False)
                except:
                    pass
        
        # FALLBACK 2: If JSON fails completely, don't crash. Return raw text.
        return {
            "final_answer_summary": "⚠️ JSON Parsing Warning (The logic is below)", 
