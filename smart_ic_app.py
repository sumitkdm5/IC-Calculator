...
# ==========================================
st.set_page_config(
    page_title="Smart Incentive Navigator", 
    layout="wide",
...
def ask_claude_json(sales_context, plan_rules_text, question, column_info="", response_format="detailed"):
    ...
    {{
        ...
        "chart_config": {{
            "type": "bar|line|combo|pie|waterfall", 
            "title": "Monthly Payout Trend",
            "data": {{"Jan": 8500, "Feb": 10000, "Mar": 4500, "Apr": 13200}},
            "x_label": "Month",
            "y_label": "Payout ($)",
            "annotations": [{{"x": "Apr", "y": 13200, "text": "Accelerator Applied"}}]
        }},
        "recommendations": [
    ...
    try:
        ...
        return {
            "final_answer_summary": "System Error - Could not reach AI engine", 
            "confidence_level": "low",
            "detailed_logic_markdown": str(e), 
            "chart_config": {},
            "key_stats": [],
            "rule_citations": [],
            "recommendations": [],
...
def process_sales_file(uploaded_file_bytes, file_name):
    ...
    try:
        ...
        columns_str = ", ".join([str(c) for c in all_columns[:50]])  # Limit column list
...
def create_plotly_chart(chart_config):
    """Create enhanced interactive Plotly charts"""
...
with st.sidebar:
    st.image("https://via.placeholder.com/200x60/0066cc/ffffff?text=Smart+IC+Navigator", use_column_width=True)
    ...
    if st.button("🗑️ Clear History", use_column_width=True):
        reset_history()
        st.rerun()
    
    if st.button("📥 Export Current Session", use_column_width=True):
    ...
    if st.session_state.favorites:
        ...
        for idx, fav in enumerate(st.session_state.favorites[-5:]):  # Show last 5
...
if st.session_state.data_loaded:
    ...
    for idx, suggestion in enumerate(suggestions[:8]):
        col_idx = idx % 4
        if cols[col_idx].button(suggestion, key=f"sugg_{idx}", use_column_width=True):
    ...
    with query_col2:
        submit_clicked = st.button("🔍 Analyze", type="primary", use_column_width=True)
    ...
    if st.session_state.history:
        ...
        for idx, item in enumerate(st.session_state.history):
            ...
            with st.container():
                ...
                with action_col3:
                    if st.button("🔄 Ask Follow-up", key=f"followup_{idx}"):
                        st.session_state.current_query = f"Based on the previous analysis about '{query}', "
                        st.rerun()
    ...
    with st.expander("🔍 Data Inspector", expanded=False):
        ...
        st.markdown("### 📄 Raw Context (for debugging)")
        st.text_area(
...
<div style='text-align: center; color: #666; font-size: 0.85rem;'>
    <p>Smart Incentive Navigator v2.0 | Powered by AWS Bedrock & Claude 3.5 Sonnet</p>
