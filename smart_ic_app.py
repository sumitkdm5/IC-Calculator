...
# ==========================================
# We grab keys from the secure cloud vault, not hardcoded text
...
# ==========================================
# 3. THE UI: HISTORY ENABLED
# ==========================================
st.set_page_config(page_title="Zaidyn Smart IC", layout="wide")
st.title("🤖 Zaidyn Smart Incentive Calculator")
...
with tab1:
    ...
    if 'current_prompt' in st.session_state:
        ...
        with st.spinner("🤖 AI is thinking..."):
            ...
            # Save to History List (Newest First)
    ...
    # We loop through the history list and print each item
    if len(st.session_state.history) > 0:
        ...
        for i, item in enumerate(st.session_state.history):
            ...
            # Use a container for each history item to keep it organized
            with st.container():
                st.markdown(f"### Q: {question}")
                st.success(f"**Answer:** {result.get('final_answer_summary')}")
                
                # Columns for Chart vs Logic
                ...
                st.markdown("---") # Divider line between history items
